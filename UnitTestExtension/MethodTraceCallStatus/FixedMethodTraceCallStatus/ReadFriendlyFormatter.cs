...
namespace SPEkit.UnitTestExtension
{
    ...
    public sealed class DefaultReadFriendlyConverter : IReadFriendlyConverter
    {
        ...
        public string Convert(FixedMethodTraceCallStatus me)
        {
            ...
            if (me.ParametersTypeName.Any())
            {
                ans.AppendJoin(",", me.ParametersTypeName);
            }
            ...
            else ans.AppendLine("This function have no custom attribute. (Or cannot identify type such as PostSharp Attribute.)");
            ans.AppendLine(sfTask.GetAwaiter().GetResult());
            ans.AppendLine("****************TRACE-END****************");
            return ans.ToString();
        }

        private async static Task<string> SessionsFormat(FixedMethodTraceCallStatus me)
        {
        ...
        private static string OneSessionFormat(KeyValuePair<object, MethodTraceCallStatusAttribute.CallSession> one,int index)
        {
            var ans = new StringBuilder();
            ...
            if (data.ReturnValue != null) ans.AppendLine($"|Return: {data.ReturnValue.ToString()}");
            else
            {
            ...
            {
                foreach (var s in ExceptionFormatter(data.exce,0).Split(Environment.NewLine,StringSplitOptions.RemoveEmptyEntries))
                {
                    ans.Append('|');
                    ans.AppendLine(s);
                }
